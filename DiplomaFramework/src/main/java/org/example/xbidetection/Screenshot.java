...
public class Screenshot {
    ...
    public List<VisualDifference> detectCBDbyPixels(Screenshot scr2){
        ...
        Mat thresholded = calculateAbsoluteDiff(baselineGray, testGray, 50);
        return detectVissDiff(thresholded,scr2,"bypixel" );
    }

    private Mat calculateAbsoluteDiff(Mat baselineGray, Mat testGray, double thresh){
        ...
        Imgproc.threshold(diff, thresholded, thresh, 255, Imgproc.THRESH_BINARY);
        return thresholded;
    }
    private List<VisualDifference> detectVissDiff(Mat thresholded, Screenshot scr2, String methodName){
    ...
    public List<VisualDifference> detectCBDbySSIM(Screenshot scr2){
        ...
        return detectVissDiff(thresholdMap, scr2, "SSIM");
    }

    public List<VisualDifference> detectCBDcombo(Screenshot scr2){
        //зчитування скріншотів
        Mat baselineImage = Imgcodecs.imread(scr2.getPath());
        Mat testImage = Imgcodecs.imread(path);
        //коригування розміру зображень
        ...
        Mat ssimMap = calculateSSIM(baselineGray, testGray, ssim);
        // Підрахунок абсолютної різниці
        Mat thresholded = calculateAbsoluteDiff(baselineGray, testGray, 25);
        //Поєднання підходів
        Mat combinedMask = new Mat();
        Core.bitwise_and(thresholded, ssimMap, combinedMask);
        return detectVissDiff(combinedMask,scr2, "combo");
    }
